...
function App() {
  ...
  const { data } = useQuery({ queryKey: ['getStockPrice'], queryFn: () => fetch(`${import.meta.env.VITE_SERVER_URL}/tickerInfo/${ticker}`).then(response => response.json()), enabled: !!ticker })
  ...
  return (
    <div className="!w-full !h-screen">
      <form className="flex flex-col" onSubmit={handleSubmit(data => setTicker(data.tickerValue))}>
        {data && <div className="flex flex-col bg-green-500">
